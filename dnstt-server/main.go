...
func acceptStreams(conn *kcp.UDPSession, privkey []byte, upstream string) error {
	...
	// Put an smux session on top of the Noise channel (optionally compressed).
	smuxConfig := smux.DefaultConfig()
	smuxConfig.Version = 2
	smuxConfig.KeepAliveTimeout = idleTimeout
	smuxConfig.MaxStreamBuffer = 1 * 1024 * 1024 // default is 65536
	sess, err := smux.Server(smuxRW, smuxConfig)
	if err != nil {
		return err
	}
	defer func() {
...
func sendLoop(dnsConn net.PacketConn, ttConn *turbotunnel.QueuePacketConn, ch <-chan *record, maxEncodedPayload int) error {
	...
	for {
		...
		if rec.Resp.Rcode() == dns.RcodeNoError && len(rec.Resp.Question) == 1 {
			...
			// Cache channel refs to avoid repeated lock acquisitions
...
func main() {
  ...
  %[1]s -udp ADDR -privkey-file server.key DOMAIN UPSTREAMADDR

Example:
  %[1]s -gen-key -privkey-file server.key -pubkey-file server.pub
  %[1]s -udp :5300 -privkey-file server.key t.example.com 127.0.0.1:8000
...
`, os.Args[0])
	...
	flag.StringVar(&udpAddr, "udp", "", "UDP address to listen on (e.g., :5300)")
	flag.BoolVar(&useCompression, "zstd", false, "enable zstd compression (requires client with -zstd flag)")
	flag.Parse()
	...
	if genKey {
		...
		// Legacy mode: -udp ADDR DOMAIN UPSTREAM
		if udpAddr != "" {
			if flag.NArg() != 2 {
				flag.Usage()
				os.Exit(1)
		...
		for _, bindaddr := range ptInfo.Bindaddrs {
			...
			listenAddr := bindaddr.Addr.String()
